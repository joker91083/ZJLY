...
class LdzzViewModel() : BaseViewModel() {
    ...
    constructor(context: Context?, mView: ILdzz) : this() {
    ...
    fun getData(type: Int, year: Int) {
        ...
        showDialog("加载中...")
        dataRepository.ldzz(auth, type, info.code ?: "", year,
    ...
    fun conversionTableData(data: ResultModel<List<LinkedTreeMap<String, Any>>>?): List<Any> {
        ...
        if (data != null) {
            ...
            val type = when (type) {
                1 -> object : TypeToken<List<LdzzModel.Xmgs>>() {}.type
                2 -> object : TypeToken<List<LdzzModel.Zzyldmj>>() {}.type
                else -> object : TypeToken<List<LdzzModel.Ldde>>() {}.type
            }
