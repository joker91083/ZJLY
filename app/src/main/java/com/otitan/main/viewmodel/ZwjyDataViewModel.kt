...
class ZwjyDataViewModel() : BaseViewModel() {

    var data: ResultModel<ZwjyModel<ZwjyModel.Sjgl>>? = null
    //请求返回列表数据集合
    ...
    constructor(context: Context?, mView: ILQuanData) : this() {
...
//        onRefresh.execute()
    ...
    fun getData(requestCode: Int) {
        ...
        auth = "Bearer $auth"
//        showDialog("加载中...")
        dataRepository.zwjyData(auth, type, dqcode, year, page, 10,keyWord,
...
//                        dismissDialog()
                        data = result as ResultModel<ZwjyModel<ZwjyModel.Sjgl>>
                        when (requestCode) {
                            1 -> {
                                items.clear()
