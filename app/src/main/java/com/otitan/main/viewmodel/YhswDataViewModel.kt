...
class YhswDataViewModel() : BaseViewModel() {

    var data: ResultModel<YhswModel<YhswModel.Sjgl>>? = null
    //请求返回列表数据集合
...
//        onRefresh.execute()
    ...
    fun getData(requestCode: Int) {
        ...
        auth = "Bearer $auth"
        dataRepository.yhswData(auth, dqcode, year, page, 10,
                object : RemoteDataSource.mCallback {
                    ...
                    override fun onSuccess(result: Any) {
                        data = result as ResultModel<YhswModel<YhswModel.Sjgl>>
                        when (requestCode) {
                            1 -> {
                                items.clear()
